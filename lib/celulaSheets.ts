...
const HEADERS = ['id_celula', 'nome_celula', 'rede', 'lider', 'telefone_lider'] as const;
...
export async function appendCelula(data: CelulaInput): Promise<CelulaRow> {
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: { values: [[row.id_celula, row.nome_celula, row.rede, row.lider, row.telefone_lider]] },
  });
  return row;
}
